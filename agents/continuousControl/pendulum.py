...
TRAINING_MODELS = [0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, "SAC", "CLAC", "DDPG", "PPO2"]
...
def train(training_tag):
    ...
    if(isinstance(training_tag, float)):
        ...
        for step in range(TRAINING_STEPS):
            ...
            data = data.append(test(model, "CLAC", training_tag, env, False, (step + 1) * TRAINING_TIMESTEPS))
            data = data.append(test(model, "CLAC", training_tag, env, 1, (step + 1) * TRAINING_TIMESTEPS))
            data = data.append(test(model, "CLAC", training_tag, env, 2, (step + 1) * TRAINING_TIMESTEPS))
        ...
        for step in range(TRAINING_STEPS):
            ...
            data = data.append(test(model, "SAC", training_tag, env, False, (step + 1) * TRAINING_TIMESTEPS))
            data = data.append(test(model, "SAC", training_tag, env, 1, (step + 1) * TRAINING_TIMESTEPS))
            data = data.append(test(model, "SAC", training_tag, env, 2, (step + 1) * TRAINING_TIMESTEPS))
    ...
    if(training_tag == "PPO2"):
        ...
        for step in range(TRAINING_STEPS):
            ...
            data = data.append(test(model, "PPO2", training_tag, env, False, (step + 1) * TRAINING_TIMESTEPS))
            data = data.append(test(model, "PPO2", training_tag, env, 1, (step + 1) * TRAINING_TIMESTEPS))
            data = data.append(test(model, "PPO2", training_tag, env, 2, (step + 1) * TRAINING_TIMESTEPS))
            
            if(SAVE_AGENTS):
                model.save("models/PPO2_" + ENVIRONMENT_NAME + "_s" + str(step) + "_i" + str(CURRENT_ITERATION) + "_ts" + str(TRAINING_TIMESTEPS))
...
if __name__ == "__main__":
    ...
    all_results.to_pickle("results/pendulum_data.pkl")
