...
def marginal_nn(ac_space, **kwargs):
    """
    :param kwargs: (dict) Extra keywords parameters for the convolutional layers of the CNN
    :return: (TensorFlow Tensor) The CNN output layer
    """

    #input_tensor = tf.placeholder("float", [256,512])
    input_tensor = tf.placeholder(dtype="float32", shape=[None, 1], name="marginal_input")
    
    #tf.placeholder(dtype=ac_space.dtype, shape=(n_batch,) + ac_space.shape, name="action_ph")

    activ = tf.nn.relu
    layer_1 = activ(linear(input_tensor, 'fc1', n_hidden=512, init_scale=np.sqrt(2)))
    layer_2 = activ(linear(layer_1, 'fc2', n_hidden=512, init_scale=np.sqrt(2)))
    #out = activ(linear(layer_2, 'fc3', ac_space.n, init_scale=np.sqrt(2)))
    return layer_2 #out
...
class BasePolicy(ABC):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, scale=False,
        ...
        with tf.variable_scope("input", reuse=False):
            ...
            #self._marginal_ph = tf.placeholder(dtype=ac_space.dtype, shape=(n_batch,) + ac_space.shape, name="marginal_ph")
            self._marginal_ph = tf.placeholder(dtype=ac_space.dtype, shape=(n_batch,1), name="marginal_ph")
    ...
    @property
    def marginal_ph(self):
        """tf.Tensor: placeholder for observations, shape (self.n_batch, ) + self.ob_space.shape."""
        return self._marginal_ph
...
class ActorCriticPolicy(BasePolicy):
    ...
    def _setup_init(self):
        ...
        with tf.variable_scope("output", reuse=True):
            assert self.policy is not None and self.proba_distribution is not None and self.value_fn is not None and self.marginal_policy is not None 
            ...
            if isinstance(self.proba_distribution, CategoricalProbabilityDistribution):
                self._marginal_action = tf.nn.softmax(self.policy)
            elif isinstance(self.proba_distribution, DiagGaussianProbabilityDistribution):
                self._marginal_action = [self.proba_distribution.mean, self.proba_distribution.std]
            elif isinstance(self.proba_distribution, BernoulliProbabilityDistribution):
                self._marginal_action = tf.nn.sigmoid(self.policy)
            elif isinstance(self.proba_distribution, MultiCategoricalProbabilityDistribution):
                self._marginal_action = [tf.nn.softmax(categorical.flatparam())
                                     for categorical in self.proba_distribution.categoricals]
            else:
                self._marginal_action = []  # it will return nothing, as it is not implemented
    ...
    def marginal_policy(self):
        ...
        return self._marginal_policy
    
    @property
    def marginal_action(self):
        """tf.Tensor: marginal output, e.g. logits."""
        return self._marginal_action
...
class FeedForwardPolicy(ActorCriticPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, layers=None, net_arch=None,
        ...
        self._kwargs_check(feature_extraction, kwargs)
        self.ac_space = ac_space
        self._marginal_approximation = None  
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            if(len(ac_space.shape) > 0):
                raise NotImplementedError("CLA2C currently only suports one dimensional Discrete() action spaces.")

            phi_latent = marginal_nn(ac_space, **kwargs) # = cnn_extractor(tf.constant([1]), **kwargs) 
            self._marginal_policy = self.pdtype.marginal_distribution_from_latent(phi_latent)
    ...
    def marginal_approximation(self, obs, state=None, mask=None, deterministic=False, action_mask=None):
        feed_dict = {self.obs_ph: obs}
        # Update the marginal approximation for use in calculating mutual information 
        feed_dict[self.marginal_ph]  =  [np.ones([1], dtype=np.float32)]
        self._marginal_approximation = self.sess.run([self.marginal_action], feed_dict)[0]

        # account for action mask in marginal approximation 
        for env_index in range(len(self._marginal_approximation)):
            env_mask = action_mask[env_index]
            self._marginal_approximation[env_index] *= env_mask
            self._marginal_approximation[env_index] / np.sum(self._marginal_approximation[env_index])

        return self._marginal_approximation
