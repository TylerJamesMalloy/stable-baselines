...
def marginal_network(action_space, layers, **kwargs):
    """
    :param scaled_images: (TensorFlow Tensor) Image input placeholder
    :param kwargs: (dict) Extra keywords parameters for the convolutional layers of the CNN
    :return: (TensorFlow Tensor) The CNN output layer
    ...
    with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
        ...
        model.add(tf.keras.layers.Flatten(input_shape=(int(1),)), **kwargs)
        for layer in layers:
            model.add(tf.keras.layers.Dense(layer, activation='relu'), **kwargs)
        model.add(tf.keras.layers.Dense(action_space.n), **kwargs)
...
def marginal_extractor(flat_observations, net_arch, act_fun):
    """
    Constructs an MLP that receives observations as an input and outputs a latent representation for the policy and
    a value network. The ``net_arch`` parameter allows to specify the amount and size of the hidden layers and how many
    of them are shared between the policy network and the value network. It is assumed to be a list with the following
    ...
    """
    latent = flat_observations
    marginal_layers = []  # Layer sizes of the network that only belongs to the marginal network

    # Build the non-shared part of the network
    latent_marginal = latent
    for idx, (phi_layer_size) in enumerate(zip(marginal_layers)):
        if phi_layer_size is not None:
            if(isinstance(phi_layer_size, tuple)):
                phi_layer_size = phi_layer_size[0]

            print("latent_marginal", latent_marginal)

            assert isinstance(phi_layer_size, int), "Error: net_arch[-1]['phi'] must only contain integers."



            latent_marginal = act_fun(linear(latent_marginal, "pi_fc{}".format(idx), phi_layer_size, init_scale=np.sqrt(2)))

    return latent_marginal

class BaseMarginal(ABC):
    """
    The base marginal object
    ...
    def __init__(self, sess, ac_space, n_env, n_steps, n_batch, reuse=False, scale=False, add_action_ph=False):
        self.n_env = n_env
        self.n_steps = n_steps
        self.n_batch = n_batch
        
        with tf.variable_scope("input", reuse=False):
            #self._obs_ph =  tf.placeholder(dtype=tf.int32, shape=(1, n_batch), name="input") # input is always 1 
            self._obs_ph, self._processed_obs = observation_input(Discrete(1), n_batch, scale=scale)
    ...
    @property
    def obs_ph(self):
        """tf.Tensor: placeholder for observations, shape (self.n_batch, ) + 1"""
        return self._obs_ph
    ...
    @abstractmethod
    def step(self, mask=None, action_mask=None):
        """
        Trains the network on an action taken by the network
...
class ActorCriticMarginal(BaseMarginal):
    """
    ...
    :param scale: (bool) whether or not to scale the input
    """
    def __init__(self, sess, ac_space, n_env, n_steps, n_batch, reuse=False, scale=False, proba_distribution=None):
        super(ActorCriticMarginal, self).__init__(sess, ac_space, n_env, n_steps, n_batch, reuse=reuse, scale=scale)

        self._pdtype = make_proba_dist_type(ac_space)
        self._marginal = None
        self._proba_distribution = proba_distribution
        self._value_fn = None
        self._marginal_action = None
        self._deterministic_action = None
        self._marginal_proba = None 
    
    def _setup_init(self):
        """Sets up the distributions, actions, and value."""
        with tf.variable_scope("output", reuse=True):
            assert self.marginal is not None and self.proba_distribution is not None
            ...
            self._marginal_action = self.proba_distribution.sample()
            self._deterministic_action = self.proba_distribution.mode()

            self._neglogp = self.proba_distribution.neglogp(self.marginal)
            if isinstance(self.proba_distribution, CategoricalProbabilityDistribution):
                self._marginal_proba = tf.nn.softmax(self.marginal)
            elif isinstance(self.proba_distribution, DiagGaussianProbabilityDistribution):
                self._marginal_proba = [self.proba_distribution.mean, self.proba_distribution.std]
            elif isinstance(self.proba_distribution, BernoulliProbabilityDistribution):
                self._marginal_proba = tf.nn.sigmoid(self.marginal)
            elif isinstance(self.proba_distribution, MultiCategoricalProbabilityDistribution):
                self._marginal_proba = [tf.nn.softmax(categorical.flatparam())
                                     for categorical in self.proba_distribution.categoricals]
            else:
                self._marginal_proba = []  # it will return nothing, as it is not implemented

            print("After setting up, marginal probability is: ", self._marginal_proba)
    ...
    @property
    def marginal(self):
        """tf.Tensor: marginal output, e.g. logits."""
        return self._marginal
    
    @property
    def marginal_proba(self):
        """tf.Tensor: parameters of the probability distribution. Depends on pdtype."""
        return self._marginal_proba
    ...
    @property
    def neglogp(self):
        """tf.Tensor: negative log likelihood of the action sampled by self.marginal."""
        return self._neglogp

    @abstractmethod
    def step(self, mask=None, deterministic=False, action_mask=None):
        """
...
class FeedForwardMarginal(ActorCriticMarginal):
    """
    ...
    def __init__(self, sess, ac_space, n_env, n_steps, n_batch, reuse=False, layers=None, net_arch=None,
                 act_fun=tf.tanh, cnn_extractor=nature_cnn, feature_extraction="cnn", proba_distribution=None, **kwargs):
        super(FeedForwardMarginal, self).__init__(sess, ac_space, n_env, n_steps, n_batch, proba_distribution=proba_distribution, reuse=reuse,
                                                scale=(feature_extraction == "cnn"))

        self._kwargs_check(feature_extraction, kwargs)
        #self.processed_obs = tf.constant([1])     # Input to the marginal is always 1
        ...
        if net_arch is None:
            if layers is None:
                layers = [64, 64]
            net_arch = [dict(phi=layers)]

        with tf.variable_scope("model", reuse=reuse):
            print("self.obs_ph, ", self.obs_ph)

            """if feature_extraction == "cnn":
                phi_latent = marginal_network(self.processed_obs, layers, **kwargs)
            else:
                #phi_latent = marginal_extractor(tf.layers.flatten(self.processed_obs), net_arch, act_fun)
                #pi_latent, vf_latent = mlp_extractor(tf.layers.flatten(self.processed_obs), net_arch, act_fun)
                # marginal_network(action_space, layers, **kwargs)
                """
           
            #phi_latent = marginal_network(action_space = ac_space, layers = layers, **kwargs)
            
            phi_latent = tf.placeholder(dtype=tf.float32, shape=(1, n_batch), name="input")
            print("phi_latent: ", phi_latent)

            #pi_latent Tensor("train_model/model/Tanh_2:0", shape=(?, 64), dtype=float32)
            #policy:  Tensor("train_model/model/pi/add:0", shape=(?, 40), dtype=float32)

            self._marginal = self.pdtype.marginal_distribution_from_latent(phi_latent, init_scale=0.01)

            print("self.marginal", self.marginal)

        self._setup_init()

    def policy_marginal(self, mask=None, action_mask=None):
        feed_dict = {self.obs_ph : np.array([int(1)])}                # {self.obs_ph: obs, self.states_ph: state, self.dones_ph: mask}
        if action_mask is not None and len(action_mask) != 0:
            feed_dict[self.action_mask_ph] = np.array(action_mask, dtype=np.float32)
        
        print(self.marginal_proba)

        if self.marginal_proba is not None:
            return self.sess.run(self.marginal_proba, feed_dict) # self.sess.run(self.policy_proba, feed_dict)
        else:
            return None 

    def step(self, action, mask=None, action_mask=None):



        return 

class MlpMarginal(FeedForwardMarginal):
    """
    ...
    def __init__(self, sess, ac_space, n_env, n_steps, n_batch, reuse=False, **_kwargs):
        super(MlpMarginal, self).__init__(sess, ac_space, n_env, n_steps, n_batch, reuse,
...
class LstmPolicy(RecurrentActorCriticPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, n_lstm=256, reuse=False, layers=None,
        ...
        if net_arch is None:  # Legacy mode
            ...
            with tf.variable_scope("model", reuse=reuse):
                extracted_features = tf.layers.flatten(1)
