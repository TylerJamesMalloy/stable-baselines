...
class ActorCriticPolicy(BasePolicy):
    ...
    def make_actor(self, obs=None, reuse=False, scope="pi"):
        ...
        if(self.is_discrete):
            # Take the soft max to turn the output into a probability distribution over the action space
            #self.policy = policy = tf.nn.softmax(policy)
            self.policy = policy = tf.layers.dense(pi_h, self.ac_space.n, activation=tf.nn.softmax)
            # Ensure the log_pi is accurately calculated for these probabilities. 
            logp_pi = tf.math.log(policy)
            logp_pi = tf.math.reduce_mean(logp_pi)
    ...
    def make_marginal(self, reuse=False, scope="phi"):
        # Predicts the marginal action distribution 
        with tf.variable_scope('phi', reuse=reuse):
            phi_h = tf.placeholder(tf.float32, shape=(1,1))
        ...
        phi_log_std = LOG_STD_MIN + 0.5 * (LOG_STD_MAX - LOG_STD_MIN) * (phi_log_std + 1)
        self.phi_std = std = tf.exp(phi_log_std)

        phi_ = phi_mu_ + tf.random_normal(tf.shape(phi_mu_)) * std
        logp_phi = gaussian_likelihood(phi_, phi_mu_, phi_log_std)
        self.phi_entropy = gaussian_entropy(phi_log_std)
        # MISSING: reg params for log and mu
        # Apply squashing and account for it in the probabilty
        deterministic_phi, phi, logp_phi = apply_squashing_func(phi_mu_, phi_, logp_phi)
        self.phi = phi
        self.deterministic_phi = deterministic_phi
        
        phi_proba = tf.nn.softmax(self.policy)
        self.phi_proba = phi_proba
        logp_phi = tf.math.log(self.phi_proba)

        return phi_proba, logp_phi
    ...
    def step(self, obs, state=None, mask=None, deterministic=False):
        if deterministic:
            print("error running deterministic")
...
class CLACPolicy(BasePolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env=1, n_steps=1, n_batch=None, reuse=False, scale=False):
        super(CLACPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=reuse, scale=scale)
...
class FeedForwardPolicy(CLACPolicy):
    """
